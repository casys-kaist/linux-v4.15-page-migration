			...
			int to_extra_count, int from_extra_count)
{
	...
	if (!from_mapping && !to_mapping) {
		...
		if (((mode & MIGRATE_ASYNC) == MIGRATE_ASYNC) && to_head &&
...
exchange_mappings:
		...
		if (to_page_mapping->a_ops->migratepage == buffer_migrate_page) {
			 ...
			 */
			if ((mode & MIGRATE_ASYNC) != MIGRATE_ASYNC)
			...
			if (PageDirty(to_page)) {
				if ((mode & MIGRATE_SYNC) != MIGRATE_SYNC)
					return -EBUSY;
	...
	EXCHANGE_PAGE_FIELD(to_page, from_page, lru_nid);
	EXCHANGE_PAGE_FIELD(to_page, from_page, lfu_nid);
